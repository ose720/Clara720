...
//외장 라이브러리 호출(import), gradle 로 설치한 라이브러리
...
public class BoardController {
    ...
    //@RequestParam("title")String title 에서 ("title")은 [클라이언트]의 name 이라는 속성로써
    //key 값을 매개변수를 전달
...
//
    ...
    public String updateBoard(
        ...
        for (int i = 0; i < board_array.size(); i++) {
            if (Long.toString(board_array.get(i).getSeq()).contentEquals(seq)) {
